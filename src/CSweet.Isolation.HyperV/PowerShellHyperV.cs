...
public static class PowerShellHyperV
{
    ...
    private const string ConfigureScript = """
        ...
        $bootstrapIso = $env:CSWEET_BOOTSTRAP_ISO
        $memoryBytes = [Int64]::Parse($env:CSWEET_MEMORY_BYTES, [Globalization.CultureInfo]::InvariantCulture)
        ...
        try {
            ...
            if (-not [String]::IsNullOrWhiteSpace($bootstrapIso)) {
                Add-VMDvdDrive -VM $vm -ControllerNumber 0 -ControllerLocation 3 -Path $bootstrapIso -ErrorAction Stop
            }
            ...
            if (-not [String]::IsNullOrWhiteSpace($bootstrapIso)) { $expectedDvdCount++ }
    ...
    public static Task<string> ConfigureAsync(
        ...
        string? artifactImagePath, string? bootstrapImagePath = null)
    {
        ...
        var environment = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["CSWEET_BOOTSTRAP_ISO"] = bootstrapImagePath ?? string.Empty
        };
...
public sealed class HyperVCommandException(string errorCode, string message) : Exception(message)
{
